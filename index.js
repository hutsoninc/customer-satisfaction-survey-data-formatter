...
const formatStringResponse = val => {
    ...
    if (!isDefined(formattedVal) || NOT_ANSWERED_REGEX.test(formattedVal)) {
        return ''
    }
...
module.exports.format = (event, context, callback) => {
    ...
    const offeredPartsAlternatives = formatStringResponse(data.offeredPartsAlternatives)

    const accurateQuote = formatStringResponse(data.accurateQuote)

    const contactedAfterPurchase = formatStringResponse(data.contactedAfterPurchase)

    const onlineSuggestions = formatStringResponse(data.onlineSuggestions)

    const comments = formatStringResponse(data.comments)

    const changesForHigherRating = formatStringResponse(data.changesForHigherRating)
